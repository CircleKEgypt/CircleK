...
//            return View();
//        }
//        public IQueryable<RptSale> GetReportSalesView(DataCenterContext db, SalesParameters Parobj)
//        {
//            return (from t in db.TransactionEntries
//                    join i in db.Items on new { ItemId = t.ItemId, StoreId = t.StoreId } equals new { ItemId = i.Id, StoreId = i.Storeid }
//                    join dp in db.Departments on new { DepartmentID = i.DepartmentId, StoreID = i.Storeid } equals new { DepartmentID = dp.Id, StoreID = dp.Storeid }
//                    join s in db.Stores on new { StoreID = i.Storeid } equals new { StoreID = s.StoreId }
//                    join sub in db.Suppliers on new { StoreID = s.StoreId, SupplierId = i.SupplierId } equals new { StoreID = sub.Storeid, SupplierId = sub.Id } into subGroup
//                    from sub in subGroup.DefaultIfEmpty()
//                    select new RptSale
//                    {
//                        ItemId = t.Id,
//                        StoreName = s.Location,
//                        StoreId = s.Id,
...
//                        TransDate = t.TransactionTime,
//                        ByDay = t.TransactionTime.HasValue ? t.TransactionTime.Value.Day : (int?)null,
//                        ByMonth = t.TransactionTime.HasValue ? t.TransactionTime.Value.Month : (int?)null,
//                        ByYear = t.TransactionTime.HasValue ? t.TransactionTime.Value.Year : (int?)null,
//                        StoreFranchise = s.Franchise,
//                        Cost = t.Cost,
//                        Price = t.Price,
//                        // ... other properties ...
//                    }).AsQueryable();
//        }
//        [HttpPost]
//        public async Task<IActionResult> Index(SalesParameters Parobj)
//        {
//            DataCenterContext db = new DataCenterContext();
//            CkhelperdbContext db3 = new CkhelperdbContext();
//            DataCenterPrevYrsContext db4 = new DataCenterPrevYrsContext();
...
//                    reportData1 = GetReportSalesView(db, Parobj)
//                    .GroupBy(d => new
//                    {
//                        StoreName = Parobj.VStoreName ? d.StoreName : null,
...
//                    .Where(g => !(g.Key.StoreName == null && g.Key.ItemLookupCode == null &&
//                    g.Key.DpName == null && g.Key.SupplierName == null && g.Key.ItemName == null &&
//                    g.Key.Date == null && g.Key.PerMonth == null && g.Key.PerYear == null && g.Key.StoreFranchise == null
//                    && g.Key.Cost == 0 && g.Key.Price == 0 && g.Key.DpId == null && g.Key.StoreId == 0
//                   )) // Exclude groups where both keys are null
//                    .Select(g => new
//                    {
//                        Total = g.Sum(d => d.Price),
//                        TotalCost = g.Sum(d => d.Cost),
//                        Price = g.Key.Price,
//                        Cost = g.Key.Cost,
//                        //TotalTax = g.Sum(d => d.Tax),
